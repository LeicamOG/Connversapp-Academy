...
const Layout: React.FC<LayoutProps> = ({
  ...
  const NavItem = ({ view, icon: Icon, label }: { view: ViewState, icon: any, label: string }) => (
    <motion.button
      ...
      className={`flex items-center w-full px-4 py-3.5 text-base font-medium transition-colors rounded-lg mb-2 relative group
        ${currentView === view
    ...
    >
      <Icon className={`w-6 h-6 ${isSidebarCollapsed ? '' : 'mr-3'}`} />
  ...
  return (
    <div className="min-h-screen bg-brand-dark text-white flex font-sans">
      ...
      >
        {/* Logo Area & Toggle */}
        <div className={`p-4 flex items-center transition-all ${isSidebarCollapsed ? 'justify-center flex-col gap-8' : 'justify-between pl-6 relative'}`}>
          <div className={`flex items-center transition-all duration-300 ${isSidebarCollapsed ? 'w-10 justify-center' : 'w-48 justify-start'}`}>
            <img
              src={isSidebarCollapsed ? "https://i.imgur.com/vAJo2nR.png" : "https://i.imgur.com/FIJkEbs.png"}
              alt="Logo"
              className={`transition-all duration-300 ${isSidebarCollapsed
          ...
          <button
            ...
            className={`hidden lg:flex p-1.5 rounded-full bg-white/5 hover:bg-white/10 text-gray-400 transition-colors ${isSidebarCollapsed ? '' : 'absolute right-4'}`}
          >
        ...
        {/* Navigation */}
        <nav className="flex-1 px-3 mt-6 overflow-y-auto overflow-x-hidden custom-scrollbar">
          {!isSidebarCollapsed && <div className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-4 px-4">Menu</div>}
